    ...
    v-form
      v-container(grid-list-xl)
        ...
        v-layout
          ...
          v-flex
            v-layout
              v-text-field.flex(v-model="updatedChild.firstName" label="First name")
              v-text-field.flex(v-model="updatedChild.lastName" label="Last name" required)
            v-layout
              v-text-field.flex(v-model="updatedChild.grade" label="Grade") 
              v-select.flex(v-model="updatedChild.gender" :items="['male', 'female']" label="Gender")
              v-menu(v-model="isSelectingBirthday" :nudge-right="40" transition="scale-transition" offset-y full-width min-width="290px")
                template(v-slot:activator="{ on }"): v-text-field.flex(v-model="updatedChild.dateOfBirth" label="Birthday" prepend-icon="fa-calendar-alt" v-on="on")
                v-date-picker(v-model="updatedChild.dateOfBirth" @input="isSelectingBirthday = false")
            v-layout
              v-flex: v-textarea(v-model="updatedChild.story" label="Story")
